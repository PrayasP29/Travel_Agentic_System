"""Agentorist MCP local discovery tool used by the hotel integration."""
...
async def _list_tools_and_call(
    ...
    """Connect to MCP server, list tools, and call the hotel search tool."""
...
def _extract_structured_results(result: Any) -> dict[str, Any]:
    ...
    structured = serialized.get("structured")
    if structured:
        return structured
    return serialized


def _is_empty_results(structured: dict[str, Any]) -> bool:
    """Determine whether the structured results contain any items."""
...
async def _run_search(payload: dict) -> dict:
    ...
    try:
        ...
        structured_results = _extract_structured_results(result)
        if _is_empty_results(structured_results):
            return {
                "status": "error",
                "provider": "agentorist",
                "tool_used": TOOL_NAME,
...
async def search_local_places(destination: str, venue: str | None = None) -> dict:
    ...
    query_target = venue or destination
    payload = {
        "vertical": "local",
